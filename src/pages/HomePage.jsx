...
const HomePage = () => {
  ...
  useEffect(() => {
    fetch("https://api.genesisonlineschool.rw/api/exams/")
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      <div className="bg-green-200 p-4 rounded-md shadow-md">
        ...
        <div className="bg-green-100 p-4 rounded-b-md shadow-md">
          <h3 className="text-md font-semibold mb-4 text-center">{secondaryTab.toUpperCase()} EXAMS VIEW</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
            {getExamsByLevel(secondaryTab).map((exam) => (
              <div key={exam.id} className="relative">
                ...
                {/* Display the related year for each exam */}
                {secondaryDropdownOpen[exam.id] && (
                  <div className="absolute top-10 left-0 w-full bg-white border rounded-lg shadow-lg z-10 p-2">
                    <p className="text-sm text-gray-700">{exam.course_name}</p>
                    {/* Use Link to navigate to ExamHomePage based on year */}
                    <Link 
                      to={`/exam/${exam.year}`} 
